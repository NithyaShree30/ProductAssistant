...
# Menu bar
    ...
    # TTS output
        tts_control = Menu(options, tearoff=0)
        options.add_cascade(label="Toggle TTS", menu=tts_control)
        tts_control.add_command(label="On", command=self.tts_toggle_on)
        tts_control.add_command(label="Off", command=self.tts_toggle_off)
    ...
    def tts_toggle_on(self):
        self.tts = True

    def tts_toggle_off(self):
        self.tts = False
